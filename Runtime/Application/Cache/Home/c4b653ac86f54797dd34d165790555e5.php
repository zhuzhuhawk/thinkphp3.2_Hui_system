...
<script type="text/javascript" src="http://www.jianjinchina.com/zui/layer/layer.js"></script>
		...
		<form class="form" action="/index.php/Home/Buy/add" method="post">
			<table class="table table-bordered  col-md-12 ">
				<tr><input class="form-control" type="hidden" name="bid" readonly="">
					...
					<td colspan="2" ><label for="">id：</label><input class="form-control" type="text" name="buyid"  value="<?php echo ($buyids); ?>">
	...
	<div class="container">
		<table class="table table-bordered table-striped col-md-12 table-hover mytable">
			...
			<?php if(is_array($data)): $i = 0; $__LIST__ = $data;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
					...
					<td>
						<a href="<?php echo U('Buy/edit/',array('bid'=>$vo['bid']));?>"><i class="icon icon-edit text-primary" ></i></a>&nbsp;
						<!-- <a id="edit" href="javascript:void();"  onclick="myedit('<?php echo ($vo["bid"]); ?>')"><i class="icon icon-edit text-primary" ></i></a>&nbsp; -->
						<a  onClick="return confirm('确定删除?'); " href="<?php echo U('Buy/del/',array('bid'=>$vo['bid']));?>"><i class="icon icon-trash text-danger" ></i></a>
					</td>
				</tr><?php endforeach; endif; else: echo "" ;endif; ?>
						<script type="text/javascript">
						function myedit(str){
							// alert(str);
							// var url = '<?php echo U("Buy/edit/");?>?bid='+str;
							// alert(url);
						   layer.open({
						        type: 2,
						        title: '修改',
						        ...
						        area : ['800px' , '520px'],
						        content: '<?php echo U("Buy/edit/");?>?bid='+str,
						        end: function () {
						                location.reload();
						            }
						    });
						...
						// $('#edit').on('click', function(){
						    // layer.open({
						        ...
						        // shadeClose: true, //点击遮罩关闭层
						        // area : ['800px' , '520px'],
						        // content: '<?php echo U("Buy/edit/",array("bid"=>$vo["bid"]));?>',
						        // end: function () {
						                // location.reload();
						            // }
						    // });
